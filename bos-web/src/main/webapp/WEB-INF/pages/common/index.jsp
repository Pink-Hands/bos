				...
				if(v1 == v2){
					...
					$.post("userAction_editPassword.action",{"password":v1},function(data){
						if(data == '1'){
							...
							//修改失败
							$.messager.alert("提示","密码修改失败，请联系管理员","error");
						}
					})
				}else {
					$.messager.alert("提示","新密码 与 确认密码 不一致！","warning");
				}
			}
		});
	});
				...
				<form name="editPasswordForm">
					<table cellpadding=3>
						<tr>
							<td>新密码：</td>
							<!-- 
							class="easyui-validatebox" 加入校验
							required="true" 非空校验
							validType:['length[4,6]'] 长度校验
	 						-->
							<td>
								<input data-options="required:true,validType:['length[4,8]']" id="txtNewPass" type="Password" class="txt01 easyui-validatebox" />
							</td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td>
								<input data-options="required:true,validType:['length[4,8]']" id="txtRePass" type="Password" class="txt01 easyui-validatebox" />
							</td>
						</tr>
					</table>
				</form>
